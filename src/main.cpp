    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // analyze where other cars are (which lane)
          ...
          for (int i = 0; i < sensor_fusion.size(); i++) {
              ...
              int car_lane = -1; // car lane unknown for the moment
              car_lane = detectCarLane(d); // car lane of the current surrounding car

              if (car_lane < 0) {
                continue;
              }
              // sensor fusion data allow us to determine the speed and thus
              // the relative position of the car to the ego vehicle
              ...
              // Analyze the surrounding and detect car ahead, left and right
              car_in_front = checkSurrounding(car_in_front, car_right, car_left, car_lane, lane, car_s, check_car_s)[0];
              car_right = checkSurrounding(car_in_front, car_right, car_left, car_lane, lane, car_s, check_car_s)[1];
              car_left = checkSurrounding(car_in_front, car_right, car_left, car_lane, lane, car_s, check_car_s)[2];
          }

          // Behavior planning:
          if (car_in_front) { // if there is a car ahead
            too_close = true; // first: reduce speed
            lane = changeLane(lane, car_left, car_right); // then: change lane
          } else { // if we can, always drive on center lane
            lane = comebackCenterLane(lane, car_left, car_right);
          }
